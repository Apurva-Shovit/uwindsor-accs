...
async def get_reports_summary(
    ...
    # Pre-fetch and map facilities, rooms, tanks, and projects to resolve them correctly
    ...
    wq_query: dict = {}
    if tank_id:
        wq_query["tank_id"] = tank_id
    if project_id:
        wq_query["project_id"] = project_id
    wq_logs = await WaterQualityLog.find(wq_query).to_list()
    for log in wq_logs:
        if not date_filter({"date": log.date}):
            continue
        fac_name, room_name, tank_name, log_fac_id = resolve_location(log.tank_id)
        if facility_id and log_fac_id != facility_id:
            continue
    ...
    inc_query: dict = {}
    if tank_id:
        inc_query["tank_id"] = tank_id
    if project_id:
        inc_query["project_id"] = project_id
    incidents = await IncidentReport.find(inc_query).to_list()
    for inc in incidents:
        if not date_filter({"date": inc.date}):
            continue
    ...
    census_query: dict = {}
    if tank_id:
        census_query["tank_id"] = tank_id
    if project_id:
        census_query["project_id"] = project_id
    censuses = await CensusEvent.find(census_query).to_list()
    for c in censuses:
        if not date_filter({"date": c.date}):
            continue
    ...
    # Project closures (status closed)
    ...
    # Collect all unique user IDs that performed the actions
    user_ids = set()
    for r in results:
        if r["performed_by"]:
    ...
    # Map IDs to names in results
    for r in results:
        uid = r["performed_by"]
        r["performed_by"] = user_name_map.get(uid, uid or "System")
...
async def get_executive_facility_summary(
    date_from: Optional[datetime] = Query(None),
    date_to: Optional[datetime] = Query(None),
    granularity: str = Query("monthly"), # day, week, biweekly, monthly, quarterly
    current: User = Depends(require_chair_or_admin),
):
    ...
    # Calculate baseline start census (sum of all changes before df_date)
    starting_fish = 0
    if df_date:
        starting_fish = sum(c.change for c in all_census if c.date < df_date)
    starting_fish = max(0, starting_fish)

    # Filter census events within date window
    window_census = []
    for c in all_census:
