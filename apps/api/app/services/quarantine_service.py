...
class QuarantineService:
    ...
    async def decide_exemption(exemption_id: str, body: ExemptionDecision, current_user: User) -> QuarantineExemption:
        ...
        if ex.status != "pending":
            raise HTTPException(409, "Exemption request has already been processed")
        ...
        # Claim the request before moving any fish. A manager double-clicking
        # "Accept & Transfer" fires several requests that all read the document
        # while it is still pending, so the status check above cannot stop them
        # on its own -- only this conditional update can, and whoever loses the
        # race transfers nothing.
        claimed = await QuarantineExemption.get_motor_collection().update_one(
            {"_id": ex.id, "status": "pending"},
            {"$set": claim},
        )
        ...
        ex.decided_by = claim["decided_by"]
        ex.decided_at = decided_at
        ex.updated_at = decided_at
        ex.updated_by = claim["updated_by"]
        if "rejection_reason" in claim:
            ex.rejection_reason = claim["rejection_reason"]
    ...
    @staticmethod
    async def _execute_exemption_transfer(ex: QuarantineExemption, current_user: User) -> None:
        """Move the exempted fish. Only ever called by the request that claimed `ex`."""
